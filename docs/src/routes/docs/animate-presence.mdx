...
<AnimatePresence mode="wait">
  <Show when={tab() === "a"} fallback={null}>
    <motion.div exit={{ opacity: 0 }} />
  </Show>
  <Show when={tab() === "b"} fallback={null}>
...
## Complex Shared-Layout Coordination

For route-like transitions, combine `AnimatePresence` mode control with shared `layoutId` and nested presence blocks:

```tsx
<AnimatePresence mode="popLayout" initial={false}>
  <Show when={activeThread()}>
    <motion.section layoutId={`thread-${activeThread()!.id}`} layout>
      <AnimatePresence propagate>
        <Show when={showMeta()}>
          <motion.div
            layout
...
See `/docs/demos` → **Nested Presence + Shared layoutId** for a full stress demo with rapid switching.
...
```tsx
import { AnimatePresence, motion } from "motion-solid";
import { createSignal, Key } from "solid-js";
