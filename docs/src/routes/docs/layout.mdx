...
Projection transforms sanitize invalid tree-scale values (for example `0`, `NaN`, `Infinity`) before composing CSS transforms, so layout animations don't emit non-finite transform strings.
