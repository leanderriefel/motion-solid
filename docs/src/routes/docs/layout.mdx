...
function Card(props: { expanded: Accessor<boolean>; onToggle: () => void }) {
  return (
    <Show
      when={props.expanded()}
      fallback={
    ...
    >
      <motion.div layoutId="card" onClick={props.onToggle} class="card-large" />
    </Show>
  );
}
```
...
For other changes, provide `layoutDependencies` (Accessors and/or plain values):
...
For single dependencies, you can use the shorthand `layoutDependency`:
...
<motion.section
  ...
  style={{ width: isExpanded() ? "28rem" : "20rem" }}
/>;
```
...
function ReorderableList() {
  ...
  return (
    <motion.ul layout layoutDependencies={[items]} class="list-none p-0">
      <AnimatePresence>
        <For each={items()}>
          {(item, index) => (
            <motion.li
...
function Tabs() {
  ...
  return (
    <motion.div layout layoutDependencies={[active]} class="tabs flex gap-2">
      <For each={tabs}>
        {(tab, index) => (
...
- Scale correction now keeps style-provided `border-radius`/`box-shadow` fallbacks aligned during projection scaling
- Transformed ancestor scales are now folded into tree projection correction for more stable nested measurements
- Shared `layoutId` handoff state is cleared after completion, preventing stale shared-element flashes during unrelated layout updates
- Layout projection now runs through explicit internal phases (`snapshot` -> `measure` -> `resolve` -> `projection`) with stale shared-state expiry checks for shared handoffs
- Retargeting while a layout animation is in flight now snapshots from the current visual state before resolving the next target, avoiding restart-from-origin jumps
- For high-complexity cases (nested presence, shared `layoutId`, scroll roots, and grid reflow), see `/docs/demos` examples: **Expandable Card** and **Filterable Grid**
